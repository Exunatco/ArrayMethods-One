...
function doubleValuesWithMap(arr) {
    let multiplied = arr.map(function (e) {
        return e * 2;
    })
    return multiplied;
}
...
function valTimesIndex(arr) {
    const timesIndexed = arr.map(function (e) {
...
function removeVowels(str) {
    ...
    splitStr.filter(function (letter) {
        let lowerCased = letter.toLowerCase();
        if (vowels.indexOf(lowerCased) == -1) {
            newArr.push(lowerCased);
        }
    });
    return newArr.join('');
}
